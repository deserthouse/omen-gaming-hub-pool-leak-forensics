# OMEN Gaming Hub 内存泄漏小考

> **OMEN Gaming Hub 在本机造成两次独立的内核内存泄漏（合计约 2.3 GB；非分页池总量一度达 3.9 GB）：一次来自它捆绑安装且卸载后残留的网络过滤器驱动，另一次由它的后台进程轮询 NVIDIA 驱动引起。本仓库记录完整的排查过程、判据与证据，另附一个"大占用但并非泄漏"的反例。**
...
> **提示**：本仓库收录的是针对 OMEN Gaming Hub 的内存泄漏故障诊断。**只想解决问题的读者：先花一分钟做[症状对照](#这是不是你遇到的问题)确认，然后直接按 [05 修复指南](docs/05-remediation-and-alternatives.md)操作——卸载什么、残留怎么清、怎么验证，全部在里面，不必通读取证文档。** 想弄清"凭什么这么判"的，再看 01–04。如果不熟悉命令行，也可以把本仓库的链接交给你的 AI 助手，让它按文档逐步讲解或代为执行；每一步都有预期输出可供核对。
